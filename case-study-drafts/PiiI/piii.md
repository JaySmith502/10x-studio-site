...
The deeper change is what you no longer have to trust. With a server-side redactor, "your data is safe" is a sentence you have to take on faith. With PiiI, it's a property you can verify: open the network tab, watch detection happen, and see that nothing leaves. <<< FILL IN: any early real-world usage signal once it exists - installs, a team that adopted it, a specific leak it caught. Until then, do not invent a number; the architectural framing carries this section. >>>

## What we'd still tell you

PiiI is deliberately scoped, and the README says so out loud. Grok isn't supported - its composer is unreachable from the content script's isolated world, and that adapter sits shelved rather than half-working. The first prompt on a fresh install waits a few seconds while the model downloads. The detection is multilingual, but the UI is English-only for now. And it isn't on the Chrome Web Store yet - you build it and load it unpacked, which is honest about where the project is: shipped and working, not yet packaged for one-click install. <<< FILL IN: the next milestone you want to commit to publicly - Web Store submission, a benchmark on detection recall, or a specific platform. Keep it to one concrete thing. >>>
